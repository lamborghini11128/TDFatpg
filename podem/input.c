...
newgate_frame01()
{
    ...
    ////  create node in frame 0
    sprintf(gate_name,"%s_f0", targv[0]);
    n = getnode(gate_name);
    //n = getnode(targv[0]); 
    ncktnode++;
    ncktnode_f0++;
    ...
    while (i < targc) {
        if (eq(targv[i],";")) break;
        sprintf(wire_name, "%s_f0", targv[i]);
        //w = getwire(targv[i]); 
        w = getwire(wire_name); 
        n->iwire[iwire_index] = w;
        w->nout++;
    ...
    *n->owire = w;
    ...
    ////  create node in frame 1
    iwire_index = 0;
    sprintf(gate_name,"%s_f1", targv[0]);
    n = getnode(gate_name);
    //n = getnode(targv[0]); 
    ncktnode++;
    ncktnode_f1++;
    ...
    while (i < targc) {
        if (eq(targv[i],";")) break;
        sprintf(wire_name, "%s_f1", targv[i]);
        //w = getwire(targv[i]); 
        w = getwire(wire_name); 
        n->iwire[iwire_index] = w; 
        //printf( "%s add wire %s at %d \n", n -> name, w -> name, iwire_index);
        w->nout++;
        iwire_index++;
        i++;
    }
...
// set ouput and input for tdf atpg
set_output_frame01()
{
    wptr w;
    int i,j;

...
/* generates a single pattern for a single fault */
podem_Moon(fault, current_backtracks, isPrimaryFault)
    fptr fault;
    int *current_backtracks;
    int isPrimaryFault;
{
    ...
    register wptr decision_tree; // the top of  LIFO stack of design_tree
    register wptr wtemp,wfault;    
    wptr test_possible_Moon();
    wptr fault_evaluate_Moon();
    long rand();
    ...
    /* Fig 7 starts here */
    /* set the initial goal, assign the first PI.  Fig 7.P1 */
    switch (set_uniquely_implied_value_Moon(fault)) {
        case TRUE: // if a  PI is assigned 
            sim();  // Fig 7.3
            sim_Moon();  // Fig 7.3
            if (wfault = fault_evaluate_Moon(fault)) forward_imply(wfault);// propagate fault effect
            if (check_test()) find_test = TRUE; // if fault effect reaches PO, done. Fig 7.10
            break;
        case CONFLICT:
            no_test = TRUE; // cannot achieve initial objective, no test
...
podem_frame01(fault, current_backtracks)
    fptr fault;
    int *current_backtracks;
{
    register int i,j, k;
    ...
    fptr fault;
{
    ...
    wptr
fault_evaluate_frame01(fault)
    fptr fault;
{
    register int i;
    register int temp1;
    register wptr w;
    register wptr w_fnext;

    if (fault->io) { // if fault is on GUT gate output
        w       = fault -> node -> owire[0]; // w is GUT output wire
        w_fnext = fault -> node -> fnext -> owire[0];
        if (w->value == U) return(NULL);
        if (fault->fault_type == 0 && w->value == 1 && w_fnext -> value == 0 ) w->value = D; // D means 1/0
        if (fault->fault_type == 1 && w->value == 0 && w_fnext -> value == 1 ) w->value = B; // B_bar 0/1
        return(w);
    }
    else { // if fault is GUT gate input
        w = fault->node->iwire[fault->index]; 
        if (w->value == U) return(NULL);
            ...
            if( w -> flag & INPUT )
                ...
                w_fnext = w -> inode[0] -> fnext -> owire[0]; 


            if (fault->fault_type == 0 && w->value == 1 && w_fnext == 0) w->value = D;
            if (fault->fault_type == 1 && w->value == 0 && w_fnext == 1) w->value = B;
            if (fault->node->type == OUTPUT) return(NULL);
            evaluate(fault->node);  // five-valued, evaluate one gate only, sim.c
            w->value = temp1;
            /* if GUT gate output changed */
    ...
    fptr fault;
{
    ...
    wptr
test_possible_Moon(fault)
    fptr fault;
{
    register nptr n;
    register wptr object_wire;
    register wptr object_wire_p1;
    register int object_level;
    register int object_level_p1;
    nptr find_propagate_gate();
    wptr find_pi_assignment();
    wptr find_pi_assignment_Moon();
    int trace_unknown_path();
    int p1_assign = 0;
    /* if the fault is not on primary output */
    if (fault->node->type != OUTPUT) {
        ...
        if (fault->node->owire[0]->value ^ U) {
            ...
            /* if GUT output is not D or D_bar, no test possible */
            if( (  fault -> io && fault -> node -> owire[0]                -> p1_value == U ) 
             || ( !fault -> io && fault -> node -> iwire[ fault -> index ] -> p1_value == U )  )
            {
                object_level_p1 = fault -> fault_type;
                if( fault -> io )
                    object_wire_p1  =  fault -> node -> owire[0];
                else
                    object_wire_p1  =  fault -> node -> iwire[ fault -> index ];
                p1_assign = 1;
            }
            else
            {
        ...
        if (fault->node->iwire[0]->value == U) {
            ...
            if ( fault -> node -> iwire[0] -> p1_value == U ) {
                object_level_p1 = fault -> fault_type;
                object_wire_p1  = fault -> node -> iwire[0];
                p1_assign = 1;
            }
            else
                return(NULL);
     ...
     * returns NULL if no PI is found.  */ 
    if( p1_assign )
        return(find_pi_assignment_Moon(object_wire_p1,object_level_p1)); 
    else
        return(find_pi_assignment(object_wire,object_level));
    ...
    fptr fault;
{
    ...
    void display_fault();
    /* if the fault is not on primary output */
    //printf("tset possible fault %s %d\n", fault -> node -> name, fault -> io);
    //display_fault(fault);
    //printf("   test possible %d \n", fault -> node -> owire[0] -> value);
    if (fault->node->type != OUTPUT) {
    ...
    int object_level;
{
    ...
    /* if not PI, backtrace to PI  Fig 9.3, 9.4, 9.5*/
        ...
        //printf("   new_object_wire %s\n", new_object_wire -> name);
    ...
    wptr
find_pi_assignment_Moon(object_wire,object_level)
    wptr object_wire;
    int object_level;
{
    register wptr new_object_wire;
    register int new_object_level;
    wptr find_hardest_control_Moon(),find_easiest_control_Moon();
    wptr wnxt;
    ...
    /* if not PI, backtrace to PI  Fig 9.3, 9.4, 9.5*/
        ...
        switch(object_wire->inode[0]->type) {
            case   OR:
            case NAND:
                if (object_level) new_object_wire = find_easiest_control_Moon(object_wire->inode[0]);  // decision gate
                else new_object_wire = find_hardest_control_Moon(object_wire->inode[0]); // imply gate
                break;
            case  NOR:
            case  AND:
                if (object_level) new_object_wire = find_hardest_control_Moon(object_wire->inode[0]);
                else new_object_wire = find_easiest_control_Moon(object_wire->inode[0]);
                break;
            case  NOT:
            case  BUF:
                new_object_wire = object_wire->inode[0]->iwire[0];
        ...
        //printf("   new_object_wire %s\n", new_object_wire -> name);
        if (new_object_wire) return(find_pi_assignment_Moon(new_object_wire,new_object_level));
        else return(NULL);
    }
}/* end of find_pi_assignment */
    ...
    nptr n;
{
     ...
     * larger input index means harder to control */
    for (i = n->nin - 1; i >= 0; i--) {
        if (n->iwire[i]->p1_value  == U) return(n->iwire[i]);
    }
    return(NULL);
}/* end of find_hardest_control */
    ...
    wptr w;
{
    ...
    if (w->flag & OUTPUT) return(TRUE); // X path exists
    for (i = 0; i < w->nout; i++) {
 ...
 * returns TRUE if we can backtrace to a PI to assign
 * returns CONFLICT if it is impossible to achieve or set the initial objective*/
set_uniquely_implied_value_Moon(fault)
    fptr fault;
{
    register wptr w;
    register int pi_is_reach = FALSE;
    ...
    /* frame 0 value */
    switch (backward_imply_Moon(w,fault->fault_type )) {
        case TRUE: pi_is_reach = TRUE; break;
        case CONFLICT: return(CONFLICT); break;
        case FALSE: break;  
    }
...
set_uniquely_implied_value_frame01(fault)
    fptr fault;
{
    register wptr w;
    register int pi_is_reach = FALSE;
    ...
    int logic_level;
{
    ...
    //printf("  current_wire no moon  %s %d \n ", current_wire -> name, logic_level );
    if (current_wire -> flag & INPUT) { // if PI
        if ( current_wire -> value != U &&  current_wire -> value != logic_level) { 
        ...
        //printf("bk imply    %s => %d\n", current_wire -> name, current_wire -> value);
        
        w = current_wire -> pvspi;
        if( w != NULL )
        {
 ...
 *returns TRUE if the backward implication reaches at least one PI 
 *returns FALSE if the backward implication reaches no PI */
backward_imply_Moon(current_wire,logic_level)
    wptr current_wire;
    int logic_level;
{
    register int pi_is_reach = FALSE;
    ...
    //printf("  current_wire %s %d \n ", current_wire -> name, logic_level );
    if (current_wire -> flag & INPUT) { // if PI
        if ( current_wire -> p1_value != U &&  current_wire -> p1_value != logic_level) { 
            return(CONFLICT); // conlict with previous assignment
        }
        current_wire -> p1_value = logic_level; // assign PI to the objective value
        current_wire -> p1_flag |= CHANGED;
        //printf("bk moon     %s => %d\n", current_wire -> name, current_wire -> p1_value);
        
        w = current_wire -> nxtpi;
        if( w != NULL )
        {
            w            -> value    = logic_level;
            w            -> flag    |= CHANGED;
        ...
        switch (current_wire->inode[0]->type) {
            ...
            /* go backward iteratively.  depth first search */
            case NOT:
                switch (backward_imply_Moon(current_wire->inode[0]->iwire[0],
                            (logic_level ^ 1))) {
                    case TRUE: pi_is_reach = TRUE; break;
                    case CONFLICT: return(CONFLICT); break;
                    case FALSE: break;
            ...
            case NAND:
                if (!logic_level) {
                    for (i = 0; i < current_wire->inode[0]->nin; i++) {
                        switch (backward_imply_Moon(current_wire->inode[0]->iwire[i],1)) {
                            case TRUE: pi_is_reach = TRUE; break;
                            case CONFLICT: return(CONFLICT); break;
                            case FALSE: break;
                        }
            ...
            case OR:
                if (!logic_level) {
                    for (i = 0; i < current_wire->inode[0]->nin; i++) {
                        switch (backward_imply_Moon(current_wire->inode[0]->iwire[i],0)) {
                            case TRUE: pi_is_reach = TRUE; break;
                            case CONFLICT: return(CONFLICT); break;
                            case FALSE: break;
                        }
            ...
            case BUF:
                switch (backward_imply_Moon(current_wire->inode[0]->iwire[0],logic_level)) {
                    case TRUE: pi_is_reach = TRUE; break;
                    case CONFLICT: return(CONFLICT); break;
                    case FALSE: break;
                }

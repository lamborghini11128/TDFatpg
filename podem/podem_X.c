...
// OAO:
// static function declaration here
static wptr fault_evaluate(fptr);
static wptr test_possible(fptr);
static wptr find_pi_assignment(wptr,int);
static wptr find_hardest_control(nptr);
static wptr find_easiest_control(nptr);
static nptr find_propagate_gate(int);
static trace_unknown_path(wptr);
static  check_test();
static  mark_propagate_tree(nptr);
static  unmark_propagate_tree(nptr);
static  set_uniquely_implied_value(fptr);
static  backward_imply(wptr,int);
static  forward_imply(wptr);
    ...
    int object_level;
{
    ...
    if (object_wire->flag & INPUT) {
        // XXX fixed assigned PI value here?
